...
async def wait_for(lst, count=1, timeout=10.0):
...
async def main():
    ...
    async with MessageBus() as bus:
        ...
        # 1. Conexao
        assert bus._ready
        logger.info("✅ 1. Conexao com Kafka estabelecida")
        ...
        logger.info("✅ 4. Serializacao/deserializacao BusMessage OK")
        ...
        ok = await wait_for(received, count=3, timeout=10.0)
        ...
        logger.info(f"✅ 5. Publish/Subscribe: {len(received)} mensagens recebidas")
        ...
        ok = await wait_for(agent_received, count=1, timeout=10.0)
        ...
        logger.info(f"✅ 6. publish_to_agent(): topico={agent_topic}")
        ...
        ok = await wait_for(broadcast_received, count=1, timeout=10.0)
        ...
        logger.info(f"✅ 7. broadcast(): recebido por {len(broadcast_received)} handler(s)")
        ...
        ok_a = await wait_for(results_a, count=1, timeout=10.0)
        ok_b = await wait_for(results_b, count=1, timeout=10.0)
        ...
        logger.info("✅ 8. Multiplos handlers no mesmo topico funcionando")
...
pathlib.Path("scripts/test_message_bus.py").write_text(content, encoding="utf-8")
print("✅ test_message_bus.py atualizado!")
print(f"   Linhas: {len(content.splitlines())}")
checks = ["wait_for", "assigned_set", "_time.monotonic", "wait_for(received", "wait_for(agent_received"]
for c in checks:
    ok = "✅" if c in content else "❌"
    print(f"   {ok} {c}")
